...
export const filter = (arr, test) => {
    ...
    for(let i = 0; i < arr.length; i++) {
        if(test(arr[i])){
            returnJson.pass.push(arr[i]);
        } else {
            returnJson.fail.push(arr[i]);
        }
    }
    return returnJson;
};
